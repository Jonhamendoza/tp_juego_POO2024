...
std::string* NivelFinal::getReglas(int* &aux) {
    // declaracion de archivos
    std::ifstream reglasNivelFinal, reglas, nacionalidades, tipo_de_visita, estado_civil, paises, visas, bienes_transportados;
    ...
    if (reglas.fail()) {
        std::cout << "Error al intentar abrir las reglas del nivel final\n";
    }
    reglasNivelFinal.open("NivelFinal/reglasNivlFinal.txt");
    if (reglasNivelFinal.fail()) {
        std::cout << "Error al intentar abrir las reglas del nivel final\n";
    }
    nacionalidades.open("NivelFinal/nacionalidades.txt");
    if(nacionalidades.fail()){
        std::cout << "Error al intentar abrir el archivo nacionalidades de Nivel final\n";
    }
    tipo_de_visita.open("NivelFinal/tipo_de_visita.txt");
    if(tipo_de_visita.fail()){
        std::cout << "Error al intentar abrir el archivo tipo de visita de nivel final\n";
    }
    estado_civil.open("NivelFinal/estado_civil.txt");
    if(estado_civil.fail()){
        std::cout << "Error al intntar abrir el archivo estado civil del nivel final\n";
    }
    paises.open("NivelFinal/paises.txt");
    if(paises.fail()){
        std::cout << "Error al intentar abrir el archivo paises del nivel final\n";
    }
    visas.open("NivelFinal/visa.txt");
    if(visas.fail()){
        std::cout << "Error al intentar abrir el archivo visa del nivel final\n";
    }
    bienes_transportados.open("NivelFinal/bienes_transportados.txt");
    if(bienes_transportados.fail()){
        std::cout << "Error al intentar abrir el archivo bienes transportados del nivel final\n";
    }
    ...
    for (int i = 0; i < 14; i++) {
        ...
        std::string aux;
        std::getline(reglasNivelFinal, aux);
    ...
    carpetaReglas[11]+="Refugiado Politico";
    carpetaReglas[12]+=Bienes_transportados[aux_Random_Tipo_de_Visita];
    return carpetaReglas;
}

void NivelFinal::getSolicitudes(std::string Solicitudes[], int &aux_TipoPersona, int* &auxSolicitudes) {
...
bool NivelFinal::verificarParametros(int *auxReglas, int *auxSolicitudes){
    bool Decision=false;
    ...
    if(auxSolicitudes[2]==tipo_de_visita)   // verificacion de estado civil
        Decision = true;
    if(auxReglas[2]==auxSolicitudes[2])     //verificacion de pais anterior 1
        Decision=true;
    if(auxReglas[3]==(auxSolicitudes[2]+10)) //verificacion de pais anterior 2
        Decision=true;
    if(auxReglas[4]==auxSolicitudes[3]) // verificacion de visa anterior 1
        Decision=true;
    if(auxReglas[4]==(auxSolicitudes[3]+3))  // verificacion de visa anterior 2
        Decision=true;
